var myApp = angular.module('myApp', ['ngRoute', 'ngCookies', 'angular-inview']);
myApp.controller('ActivitiesController', ['$scope', '$http', '$location', '$routeParams', '$cookies', function($scope, $http, $location, $routeParams, $cookies) {
    ...
    $scope.post_activity = function() {
        ...
        //then
        //trim spaces
        var tags_trimmed = $('#category .btn').attr('title').split(',').map(function(item) {
            return item.trim()
        });
        $http({
    ...
    $scope.post_sportscenter = function() {
        ...
        var shop_tags = $('#category_spor .btn').attr('title').split(',').map(function(item) {
            return item.trim()
        });
    ...
    $scope.put_activities = function() {
        ...
        if ($scope.checked && $scope.checked2) {
            var activity_tags = $('#category_act .btn').attr('title').split(',').map(function(item) {
                return item.trim()
            });
            $http({
                method: "POST",
    ...
    $scope.getActivity = function() {
        param_array = getParams(window.location.href)
        var search_prod = param_array.search_prod
        var search_shop = param_array.search_shop
        $scope.selected_price = param_array.sel_price
        console.log(param_array);
        $http({
            url: 'https://localhost:8765/observatory/api/products/' + search_prod,
            method: "GET"
        }).then(function(response2) {
            $scope.activity = response2.data;
            console.log($scope.activity);
            $http({
                url: 'https://localhost:8765/observatory/api/shops/' + search_shop,
                method: "GET"
            }).then(function(response3) {
                $scope.shop = response3.data;
                console.log($scope.shop);
                $scope.map($scope.shop.lng, $scope.shop.lat)
                $http({
                    url: 'https://localhost:8765/observatory/api/prices/activitysportscenter?shop=' + search_shop + '&product=' + search_prod,
                    method: "GET"
    ...
    $scope.map = function(a, b) {
        var chania = new ol.Feature({
            geometry: new ol.geom.Point(ol.proj.fromLonLat([a, b])),
            type: 'circle'
        })
        var styles = {
        ...
        //creation of feautures-list == all the markers we want to show
        var vectorSource = new ol.source.Vector({
            features: [chania]
        });
        //create leayer with these features and this style
        ...
        const map = new ol.Map({
            target: 'map',
            layers: [
                new ol.layer.Tile({ //this is the basic layer
                    source: new ol.source.OSM()
                }),
                vectorLayer //this is our customized layer
            ],
            view: new ol.View({
                center: ol.proj.fromLonLat([a, b]), //takes [long, lat]
                zoom: 15
            })
        });
    }
    ...
    $scope.autocomplete_val = ""
    $scope.search_activity = function() {
        window.location.href = "/search?search_val=" + $scope.autocomplete_val;
    }
    $scope.give_query = function() {
        var search_val = window.location.search.substring(window.location.search.indexOf('search_val') + 'search_val'.length + 1); //get query parameter
        ...
        var stringinput = 'https://localhost:8765/observatory/api/prices?count=10000&products=' + search_val + '&';
        ...
        if ($scope.fromDate != null && $scope.toDate != null) {
            ...
            now_date = now_date.toISOString().split('T')[0]
            stringinput = stringinput + 'dateFrom=' + from + '&dateTo=' + now_date + '&'
        }
        ...
        if ($scope.selected_sort != null) {
            if ($scope.selected_sort == 'Τιμή Αύξουσα') {
                stringinput = stringinput + 'sort=price%7CASC';
            } else if ($scope.selected_sort == 'Τιμή Φθίνουσα') {
                stringinput = stringinput + 'sort=price%7CDESC';
            } else if ($scope.selected_sort == 'Απόσταση Αύξουσα') {
                stringinput = stringinput + 'sort=geoDist%7CASC';
            } else {
                stringinput = stringinput + 'sort=geoDist%7CDESC';
            }
        }
        if (stringinput.slice(-1) == '&') {
            stringinput = stringinput.substring(0, stringinput.length - 1);
        }
        var done_geocoding = function(response) {
            console.log('IN GEOCODING')
            stringinput = stringinput + '&geoDist=' + $scope.distance.toString() + '&geoLng=' + response.data[0].lon + '&geoLat=' + response.data[0].lat + '&'
            ...
            $http({
                ...
                if (response.data.prices) {
                    var temp = groupBy(response.data.prices, function(item) {
                        return [item.dateFrom, item.dateTo, item.user, item.shopId, item.productId];
                    });
                    if ($scope.price_per_day != null) {
                        $scope.results = temp.filter(function(item) {
                            return (item.price <= $scope.price_per_day)
                        });
                    } else {
                        $scope.results = temp
                    }
        ...
        if ($scope.distance != null) { //XRHSTO TIS SYNTETAGMENES!
            //herere
            $http({
                url: 'https://nominatim.openstreetmap.org/search/' + $scope.address,
                method: "GET",
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    $scope.init_search = function() {
        ...
        var stringinput = 'https://localhost:8765/observatory/api/prices?count=10000&dateFrom=2016-05-05&dateTo=' + now_date + '&products=' + search_val;
        /*if($scope.price_per_day!=null) {
            stringinput=stringinput+'price='+$scope.price_per_day.toString();+'&'} */
        if ($scope.fromDate != null && $scope.toDate != null) {
            var dd = $scope.fromDate.getDate();
        ...
        $http({
            ...
            $scope.results = groupBy(response.data.prices, function(item) {
                return [item.dateFrom, item.dateTo, item.user, item.shopId, item.productId];
            });
            console.log($scope.results);
        });
...
myApp.filter('conv_day', [function() {
    return function(n) {
        n = n.split("T");
        return n[0]
    };
